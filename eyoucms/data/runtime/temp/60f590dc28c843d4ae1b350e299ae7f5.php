<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:46:"./application/admin/template/arctype\index.htm";i:1545616313;s:63:"E:\www\www\eyoucms\application\admin\template\public\layout.htm";i:1547521060;s:63:"E:\www\www\eyoucms\application\admin\template\public\footer.htm";i:1545615936;}*/ ?>
<!doctype html>
<html>
<head>
  ...
  <form method="post">
    <div class="flexigrid">
      <div class="mDiv">
        <div class="ftitle">
          <h3>栏目列表</h3>
          <h5>(共<?php echo count($arctype_list); ?>条记录)</h5>
        </div>
        <div title="刷新数据" class="pReload"><i class="fa fa-refresh"></i></div>
        <?php if($main_lang == $admin_lang): ?>
        <div class="fbutton">
              <a href="<?php if(in_array(\think\Request::instance()->action(), array('add','edit'))): ?>javascript:void(0);<?php else: ?><?php echo U("Arctype/add"); endif; ?>">
                  <div class="add">
              ...
              <tr>
                ...
                <th axis="col3" class="">
                  <div class="sundefined" style="padding-left: 15px">
                    <img src="/eyoucms/public/static/admin/images/tv-expandable.gif" id="all_treeclicked" title="关闭所有子栏目" style="float: none;" data-status="open" onClick="treeClicked(this,'all',0);">
                    栏目名称
                  </div>
                </th>
            ...
            <tr nctype="0" <?php if($vo['parent_id'] > 0): ?> style="display:none;"<?php endif; ?> class="parent_id_<?php echo $vo['parent_id']; ?>" data-level="<?php echo $vo['level']; ?>" data-id="<?php echo $vo['id']; ?>">
              ...
              <td class="typename" style="width: 100%">
                <div>
                  ...
                  <img src="/eyoucms/public/static/admin/images/tv-expandable.gif" style="float: none;" fieldid="2" status="open" nc_type="flex" onClick="treeClicked(this,<?php echo $vo['id']; ?>,0);" class="has_children">
                  <?php else: ?>
                  <img src="/eyoucms/public/static/admin/images/tv-collapsable-last.gif" style="float: none;" fieldid="2" status="open" nc_type="flex">
                  <?php endif; if($vo['current_channel'] == 6): ?>
                  <a href="<?php echo U('Arctype/single_edit',array('typeid'=>$vo['id'])); ?>"><?php echo $vo['typename']; ?></a>
                  <?php else: ?>
                  <a href="<?php echo U($channeltype_list[$vo['current_channel']]['ctl_name'].'/index',array('typeid'=>$vo['id'], 'tab'=>3)); ?>"><?php echo $vo['typename']; ?></a>
              ...
              <td>
                  <div class="w300 tl pb0">
                      <?php if(is_check_access('Archives@index') == '1'): if($vo['current_channel'] == 6): ?>
                        <a href="<?php echo U('Arctype/single_edit',array('typeid'=>$vo['id'])); ?>" class="btn blue"><i class="fa fa-pencil-square-o"></i>内容</a>
                        <?php else: ?>
                        <a href="<?php echo U($channeltype_list[$vo['current_channel']]['ctl_name'].'/index',array('typeid'=>$vo['id'], 'tab'=>3)); ?>" class="btn blue"><i class="fa fa-pencil-square-o"></i>内容</a>
                        <?php endif; endif; ?>
                      <a href="<?php echo U('Arctype/edit',array('id'=>$vo['id'])); ?>" class="btn blue"><i class="fa fa-pencil-square-o"></i>编辑</a>
                      <?php if(is_check_access('Arctype@add') == '1'): if($main_lang == $admin_lang): if($vo['grade'] < ($arctype_max_level - 1)): ?>           
                          <a href="<?php echo U('Arctype/add',array('parent_id'=>$vo['id'])); ?>" class="btn blue"><i class="fa fa-pencil-square-o"></i>增加子栏目</a>
                        <?php else: ?>
                          <a class="btn blue" title="不支持增加四级栏目"><i class="fa fa-pencil-square-o"></i>不支持增加</a>
                        <?php endif; endif; endif; if(is_check_access('Arctype@del') == '1'): if($main_lang == $admin_lang): ?>
                      <a class="btn red"  href="javascript:void(0);" data-url="<?php echo U('Arctype/del'); ?>" data-id="<?php echo $vo['id']; ?>" data-typename="<?php echo $vo['typename']; ?>" onClick="delfun(this);"><i class="fa fa-trash-o"></i>删除</a>
                      <?php endif; endif; ?>
              ...
              <td class="sort">
                <div class="w60 tc pb0">
                  ...
                  <input type="text" onKeyUp="this.value=this.value.replace(/[^\d]/g,'');" onpaste="this.value=this.value.replace(/[^\d]/g,'');" onChange="changeTableVal('arctype','id','<?php echo $vo['id']; ?>','sort_order',this);" size="4" value="<?php echo $vo['sort_order']; ?>" class="tc" />
                  <?php else: ?>
  ...
  <script type="text/javascript">
    ...
    function treeClicked(obj,id,reload){
        if (id == 'all') {
          ...
          if (status == 'close') {
            $('tr[class^=parent_id_]').show().find('img').attr('src', '/eyoucms/public/static/admin/images/tv-collapsable-last.gif');
            $(obj).attr('data-status', 'open').attr('title','关闭所有子栏目').attr('src','/eyoucms/public/static/admin/images/tv-collapsable-last.gif');
          } else {
            ...
            $(obj).attr('data-status', 'close').attr('title','展开所有子栏目').attr('src','/eyoucms/public/static/admin/images/tv-expandable.gif');
          }
         ...
         if(src == '/eyoucms/public/static/admin/images/tv-expandable.gif')
         {
             // $("#treet1 tr").removeClass('tb_child');
             // $(".parent_id_"+id).addClass('tb_child');
             $(".parent_id_"+id).show();
             $(obj).attr('src','/eyoucms/public/static/admin/images/tv-collapsable-last.gif');
             var status = 'close';
         }else{
             $(obj).attr('src','/eyoucms/public/static/admin/images/tv-expandable.gif');     
             var status = 'open';      
              ...
              for (i = 0; i < tbl.rows.length; i++)
              {
                  ...
                  else
                  {
                      if (fnd == true)
                      {
                          ...
                          {
                              $(row).hide();        
                              $(row).find('img.has_children').attr('src','/eyoucms/public/static/admin/images/tv-expandable.gif');
                          }
                          else
                          {
                              fnd = false;
